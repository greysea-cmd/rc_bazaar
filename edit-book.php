<?php
// edit-book.php - Edit existing book listing
...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    flash_message('Book ID is required.', 'error');
...
if ($book_data['seller_id'] != $user_id) {
    flash_message('You are not authorized to edit this book.', 'error');
    redirect('my-books.php');
}

// Check if book can be edited (only approved books can be edited)
if ($book_data['status'] !== 'approved') {
    flash_message('Only approved books can be edited. Current status: ' . ucfirst($book_data['status']), 'error');
    redirect('my-books.php');
}

$categories = $category->getAllCategories();
$errors = [];
$form_data = $book_data; // Pre-fill form with existing data
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $form_data = [
        ...
        'image_url' => $book_data['image_url'] // Keep existing image by default
    ];
    ...
    if (isset($_FILES['book_image']) && $_FILES['book_image']['error'] === UPLOAD_ERR_OK) {
        try {
            ...
            if (!empty($form_data['image_url'])) {
                $old_image_path = UPLOAD_PATH . $form_data['image_url'];
    ...
    if (empty($errors)) {
        if ($book->update($form_data)) {
            flash_message('Book updated successfully!', 'success');
            redirect('my-books.php');
        } else {
        ...
        <main class="main-content">
            ...
            <div class="form-section">
                ...
                <div class="current-image-container">
                    <p class="form-label">Current Book Image:</p>
                    <?php if (!empty($form_data['image_url'])): ?>
                        <img src="<?php echo UPLOAD_PATH . htmlspecialchars($form_data['image_url']); ?>" 
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" step="0.01" min="0.01" class="form-control" id="price" name="price"
                                value="<?php echo number_format($form_data['price'] ?? 0, 2, '.', ''); ?>" required>
                        </div>
                        <div class="col-md-4 mb-3">
                    ...
                    <div class="mb-3">
                        <label for="book_image" class="form-label">Update Book Image (Optional)</label>
                        <input type="file" class="form-control" id="book_image" name="book_image" accept="image/*">
                        <small class="text-muted">Upload a new image to replace the current one (JPG, PNG, GIF - Max 5MB)</small>
                    </div>

                    <div class="d-flex justify-content-between gap-2">
                        <div>
                            <a href="my-books.php" class="btn btn-secondary">Cancel</a>
                            <a href="book-details.php?id=<?php echo $book_id; ?>" class="btn btn-secondary">
